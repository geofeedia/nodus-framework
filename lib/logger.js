...
function Logger(options) {
    ...
    const log_name = options.name || get_name();
    const log_level = options.level ? options.level.toUpperCase() : DEFAULT_LEVEL;

    const level_index = level => LOG_LEVELS.indexOf(level.toUpperCase());
    const is_error = msg => msg.level === 'ERROR';

    const logger = {
        name: log_name,
        level: log_level,
        log: msg => {
            // ** Forward to logger events
    ...
    _.each(
        _.map(LOG_LEVELS, level => level.toLowerCase()),
        msg_level => {
            logger[msg_level] = function () {
                if (level_index(msg_level) <= level_index(log_level))
                    logger.log(extend(true, {level: msg_level}, Message(arguments)));
            }
        });
